...
async function startListening(api) {
  ...
  logger.log(
    `⫸ TBH ➤ [ LOADED ] [ BOT_INFO ] success!`,
    "LOADED"
  );

  try {
    const Threads = require('./includes/database/threads')({ api });
    ...
    if (Array.isArray(allThreads)) {
      for (const thread of allThreads) {
        try {
          ...
          // Enable ReactUnsend if not already enabled
          if (threadData.reactUnsend === undefined || threadData.reactUnsend === false) {

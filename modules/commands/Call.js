...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (!number || !/^01[0-9]{9}$/.test(number)) {
    return api.sendMessage(
      "অনুগ্রহ করে সঠিক বাংলাদেশি নাম্বার দিন (উদাহরণ: /call 01xxxxxxxxx) দয়া করে কেউ খারাপ কাজে ব্যবহার করবেন না 🙂,\n ফাইলটি শুধুমাত্র মজা করার উদ্দেশ্যে তৈরি করা হয়েছে।", 
      event.threadID, 
      event.messageID
    );
  }
  ...
  api.sendMessage(
    ...
    async (err, info) => {
      try {
        ...
        return api.sendMessage(`🫦 client ke kheye deoya hocche ${number} নম্বরে।`, event.threadID, event.messageID);
      } catch (error) {
